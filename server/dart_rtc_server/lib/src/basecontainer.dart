...
class BaseContainer<T> extends GenericEventTarget<ContainerEventListener> {
  ...
  void add(T e) {
    if (!_list.contains(e)) {
      _list.add(e);
      listeners.where((l) => l is ContainerContentsEventListener).forEach((l) {
        (l as ContainerContentsEventListener).onCountChanged(this);
      });
    }
  }
  ...
  void remove(T e) {
    if (_list.contains(e)) {
      if (_list.removeAt(_list.indexOf(e)) != null) {
        listeners
            .where((l) => l is ContainerContentsEventListener)
            .forEach((l) {
          (l as ContainerContentsEventListener).onCountChanged(this);
        });
      }
