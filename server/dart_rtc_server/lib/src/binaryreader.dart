...
class BinaryReader {
  ...
  */
  void readChunk(Uint8List buf) {
    ByteData v = buf.buffer.asByteData();
    ...
    for (int i = 0; i < chunkLength; i++) {
      ...
      if (_currentReadState == BinaryReadState.READ_CONTENT) {
        if (v.getUint8(i) == NULL_BYTE &&
            v.getUint8(i + 1) != null &&
            v.getUint8(i + 2) == NULL_BYTE) {
          _process_end();
        } else {
  ...
  void _process_init_read(int b) {
    if (b == FULL_BYTE) _currentReadState = BinaryReadState.READ_TYPE;
  }
   ...
   */
  void _process_read_length(int index, ByteData dv) {
  ...
  void _processBuffer() {
    try {
      Packet? p =
          PacketFactory.getPacketFromString(new String.fromCharCodes(_buffer));
      // _signalReadPacket(p);
...
class BinaryReadState {
  ...
  operator ==(Object o) {
    if (!(o is BinaryReadState)) return false;

    BinaryReadState brs = o;
