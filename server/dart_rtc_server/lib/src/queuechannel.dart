...
class QueueChannel extends Channel implements UserConnectionEventListener {
  ...
  bool join(User u) {
    ...
    //  return super.join(u);
    //} else {
    if (userCount == 0) return super.join(u);
  ...
  void leave(User u) {
    super.leave(u);
    if (_isInQueue(u)) _removeFromQueue(u);
  ...
  void _removeFromQueue(User u) {
    ...
    if (index > -1) _queue.removeAt(index);

    listeners.where((l) => l is ChannelQueueEventListener).forEach((l) {
      (l as ChannelQueueEventListener).onLeaveQueue(this, u);
    });
    ...
    if (_queue.length > 0) {
      for (int i = 0; i < _queue.length; i++) {
        User qu = _queue[i];
        listeners.where((l) => l is ChannelQueueEventListener).forEach((l) {
          (l as ChannelQueueEventListener)
              .onMoveInQueue(this, qu, _queue.length, i);
        });
      }
    }
  }
  ...
  void _insertIntoQueue(User u) {
    ...
    listeners.where((l) => l is ChannelQueueEventListener).forEach((l) {
      (l as ChannelQueueEventListener)
          .onEnterQueue(this, u, _queue.length, _queue.indexOf(u));
    });
  }
